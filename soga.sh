...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/amfiyong/wallow/master/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/amfiyong/wallow/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/soga -N --no-check-certificate https://github.com/amfiyong/wallow/raw/master/soga.sh
    if [[ $? != 0 ]]; then
        echo ""
...
--- https://github.com/amfiyong/wallow ---
  ${green}0.${plain} 退出脚本
————————————————
